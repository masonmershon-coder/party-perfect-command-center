/**
 * MATTER — access layer for the Party Perfect AI OS shared contract (P1).
 *
 * NEUTRAL infrastructure. It knows DOMAINS, not personas. "Mike" (party_perfect)
 * and "Matter" (mershon_personal) are app-layer names; this file never hardcodes them.
 *
 * AGENT ≠ DOMAIN:
 *   - executor/actor  = the WORKER (claude, cursor, chatgpt, grok, human, gateway…)
 *   - domain          = whose CONTEXT + authority the work belongs to (party_perfect,
 *                       mershon_personal, mershon:<member>)
 * The same worker (e.g. Claude) can operate in any domain; the domain decides scope.
 *
...
const g = globalThis as unknown as { __matterPool?: pg.Pool };
function db(): pg.Pool {
  if (!process.env.DATABASE_URL?.trim()) throw new Error("matter: DATABASE_URL not set");
  if (!g.__matterPool) g.__matterPool = new pg.Pool({ connectionString: process.env.DATABASE_URL, max: 3 });
  return g.__matterPool;
}
export function isMatterConfigured(): boolean {
  return Boolean(process.env.DATABASE_URL?.trim());
}
function requireDomain(domain: string | undefined | null): string {
  const d = (domain || "").trim();
  if (!d) throw new Error("matter: domain is required (no default — pass party_perfect / mershon_personal / mershon:<member>)");
  return d;
}
...
export async function updateTask(id: string, patch: Partial<{ status: TaskStatus; assignedExecutor: Executor; result: string; outputArtifacts: unknown[]; executionMode: ExecutionMode }>): Promise<void> {
  ...
  vals.push(id);
  await db().query(`update matter.tasks set ${sets.join(", ")} where id = $${i}`, vals);
}
...
export async function listTasks(filter: { domain: Domain; status?: TaskStatus; executor?: Executor; limit?: number }): Promise<Record<string, unknown>[]> {
  ...
  const { rows } = await db().query(`select * from matter.tasks where ${where.join(" and ")} order by created_at desc limit $${i}`, vals);
  return rows;
}
...
export async function createApproval(a: { domain: Domain; taskId?: string; summary: string; risk?: "external" | "financial" | "irreversible" | "deletion" | "other" }): Promise<{ id: string }> {
  ...
  const { rows } = await db().query(
    `insert into matter.approvals (domain, task_id, summary, risk) values ($1,$2,$3,$4) returning id`,
...
export async function decideApproval(id: string, decision: "APPROVED" | "REJECTED", decidedBy: string, note?: string): Promise<void> {
  const { rows } = await db().query(
    `update matter.approvals set status=$2, decided_by=$3, decided_at=now(), note=$4 where id=$1 returning domain`,
...
export async function upsertBrainRecord(r: {
  ...
  const { rows } = await db().query(
    `insert into matter.brain_records
...
export async function listBrainRecords(filter: { domain: Domain; status?: BrainStatus; limit?: number }): Promise<Record<string, unknown>[]> {
  ...
  if (filter.status) { where.push(`status = $${i++}`); vals.push(filter.status); }
  vals.push(Math.min(filter.limit ?? 100, 500));
  const { rows } = await db().query(`select * from matter.brain_records where ${where.join(" and ")} order by created_at desc limit $${i}`, vals);
  return rows;
}
...
export async function indexArtifact(a: { domain: Domain; kind?: string; location?: string; pathOrUrl?: string; sha256?: string; bytes?: number; relatedTask?: string; relatedMeeting?: string }): Promise<{ id: string }> {
  const domain = requireDomain(a.domain);
  const { rows } = await db().query(
    `insert into matter.artifacts (domain, kind, location, path_or_url, sha256, bytes, related_task, related_meeting)
...
export async function logAudit(e: { domain: Domain; actor?: string; action: string; entityType?: string; entityId?: string; detail?: Record<string, unknown> }): Promise<void> {
  ...
  await db().query(
    `insert into matter.audit_log (domain, actor, action, entity_type, entity_id, detail) values ($1,$2,$3,$4,$5,$6::jsonb)`,
